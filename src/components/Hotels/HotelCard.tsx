...
const HotelCard: React.FC<HotelCardProps> = ({hotelData}) => {
  ...
  return (
    <Link href={`/hotels/${hotelData._id}`} passHref  className="group relative w-80 md:w-full h-80 md:h-80 shadow-lg text-white rounded-lg overflow-hidden hover:scale-105 transition">
      
        <Image src={hotelData.cover_image} loading="lazy" className="w-full h-full object-cover" width={100} height={100} alt="" />
        <div className="absolute bottom-0 left-0 right-0 py-2 px-4 flex flex-col bg-black bg-opacity-20 group-hover:bg-opacity-40">
          <h4 className="text-md font-bold">{hotelData.property_name}</h4>
          <p className="text-xs font-semibold">{hotelData.property_address.street_address}</p>
        </div>
